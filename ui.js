...
const STAMP_SECTION_BRUSH_NAMES = 'boid ant bristle simple eraser';
...
export function buildSidebar(app) {
  ...
  sb.innerHTML = `
    ...
    <!-- Blob Fluid (blob only) -->
    <div class="section-header" data-brushes="blob" data-section="blobFluid">Blob Fluid <span class="chevron">▼</span></div>
    <div class="section-body" data-brushes="blob">
      ${sliderRow('blobSize', 'Blob Size', 12, 320, 96)}
      ${sliderRow('blobResolution', 'Resolution', 16, 96, 48, null, 'Grid resolution for the internal density simulation bitmap')}
      ${sliderRow('blobSoftness', 'Mask Softness', 0, 100, 35, v => (v / 100).toFixed(2), 'Softens the outer procedurally generated blob mask')}
      ${sliderRow('blobIterations', 'Fluid Iter.', 1, 24, 8, null, 'Number of density-simulation passes inside each blob')}
      ${sliderRow('blobContrast', 'Contrast', 50, 300, 140, v => (v / 100).toFixed(2), 'Expands or compresses the simulated density values')}
      ${sliderRow('blobThreshold', 'Threshold', 0, 90, 18, v => (v / 100).toFixed(2), 'Cuts away low-density regions inside the blob')}
      ${sliderRow('blobColorMix', 'Color Map', 0, 100, 70, v => (v / 100).toFixed(2), 'How strongly density modulates the brush color')}
      ${sliderRow('blobOpacity', 'Opacity', 1, 100, 85, v => (v / 100).toFixed(2))}
      ${sliderRow('blobSpacing', 'Spacing', 5, 100, 30, v => v + '%', 'Distance between repeated blob stamps as a percent of blob size')}
      <label>Press→Size <input type="checkbox" id="blobPressureSize" checked></label>
      <label>Press→Opac <input type="checkbox" id="blobPressureOpacity" checked></label>
    </div>

    <!-- Stamp -->
    <div class="section-header" data-brushes="${STAMP_SECTION_BRUSH_NAMES}" data-section="stamp">Stamp <span class="chevron">▼</span></div>
    <div class="section-body" data-brushes="${STAMP_SECTION_BRUSH_NAMES}">
...
const _sliderFormats = {
  ...
  // Blob fluid
  blobSoftness: v => (v / 100).toFixed(2),
  blobContrast: v => (v / 100).toFixed(2),
  blobThreshold: v => (v / 100).toFixed(2),
  blobColorMix: v => (v / 100).toFixed(2),
  blobOpacity: v => (v / 100).toFixed(2),
  blobSpacing: v => v + '%',
  // AI diffusion
