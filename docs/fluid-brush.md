...
The Fluid Brush is the **legacy free-flow LBM brush** backed by the fluid WASM solver from the playground. It injects pigment into a lattice-Boltzmann flow field and continuously deposits the evolving fluid frame onto the active layer. There is no blob boundary mode in the main app implementation.

A separate **3D Fluid** brush now exists for the heavier WebGPU architecture. This document remains the behavior contract for the original `FluidBrush` / `FluidSim` pathway only.
...
3. **Deposit** — Each animation frame reads the current fluid pixels from WASM, rebuilds the target layer from the captured pre-stroke layer snapshot, and draws the current fluid frame onto that layer.
...
- The startup defaults are tuned toward **faster settling**: lighter injection, stronger viscosity/decay, a slightly coarser fluid grid, and a higher stop threshold so a click-sized dab settles and commits within a few seconds.
...
- The legacy stroke contract is unchanged: `onDown` resets/captures state, `_seedAt` injects particles, `_step` advances the solver, and `_depositFrameFromSim` rebuilds the layer from the pre-stroke snapshot before compositing the current fluid frame.
