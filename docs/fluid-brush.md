...
1. **Deposit** — On each stroke sample the brush injects a burst of new particles near the cursor. Each particle is given initial velocity in the stroke direction plus an optional lateral (sideways) component.
2. **Drag** — The brush radius acts as a force field. Particles already in range receive a push in the stroke direction plus a swirling sideways component, so the pool deforms as the brush moves.
3. **Physics step** — Every frame, all live particles are advected: velocity is blended toward the local cell-average (viscosity), random turbulence is added (spread), velocity is scaled down (damping), and each particle moves by its velocity. Particles that drift off-canvas bounce back with reduced speed.
4. **Stamping** — After each physics step the position trail of each particle is drawn onto the active layer as a series of stamps, scaled by wetness, pressure, and stamp size.
...
|---|---|---|---|---|
| **Drops** | `fluidParticleLimit` | 10 – 2000 | 220 | Maximum live particles at once. More drops = richer bleed but heavier CPU cost. |
| **Emit** | `fluidEmitRate` | 1 – 100 | 12 | Particles spawned per stroke sample. Higher = thicker, more saturated deposit. |
| **Brush Radius** | `fluidBrushRadius` | 2 – 400 | 38 px | The influence radius of the brush. Particles inside this radius are pushed, twisted, and given a lateral impulse on each sample. Scales with the global Brush Scale slider. |
| **Brush Force** | `fluidBrushForce` | 0 – 6× | 0.85× | Strength of the forward push applied to particles in range. At 0 the brush deposits without dragging. At high values the pool gets flung ahead of the stroke. |
| **Lateral Spread** | `fluidLateralSpread` | 0 – 400 | 40 | How far fluid fans sideways from the stroke direction **during the stroke**. At 0 the old behaviour (no in-stroke lateral motion) is restored. At 40 the pool broadens visibly as you paint. At 200+ the result is wide and splashy. |
| **Flow Speed** | `fluidFlow` | 0.01 – 5.00 | 1.00 | Multiplier on particle advection distance per frame. Low values make the fluid sluggish; high values let it race across the canvas. |
| **Viscosity** | `fluidViscosity` | 0.00 – 1.00 | 0.45 | How strongly each particle's velocity is blended toward the local cell average. High viscosity gives a smooth, coherent smear; low viscosity allows chaotic individual streaks. |
| **Damping** | `fluidVelocityDamping` | 0.01 – 1.00 | 0.88 | Velocity decay per frame (applied as `v × damping^dt`). Values near 1.00 let particles coast for a long time; lower values stop them quickly. |

### Fluid Surface section (collapsed by default)

| Control | ID | Range | Default | Description |
|---|---|---|---|---|
| **Deposit** | `fluidDeposit` | 0.01 – 1.00 | 0.70 | Opacity multiplier for each stamp. Lower values give a more transparent, glazed look. Combines with the global Stamp Opacity. |
| **Spread** | `fluidSpread` | 0 – 400 | 18 | Random turbulence added to every particle's velocity each frame. Low values keep the pool coherent; high values break it into chaotic spatter. Also controls the width of the initial lateral scatter on new particles. |
| **Drying** | `fluidEvaporation` | 0.001 – 0.300 | 0.012 | Rate at which wetness decays. High values dry the pool quickly; very low values leave fluid alive on canvas for seconds. |
| **Texture Flow** | `fluidTextureFollow` | 0.00 – 1.00 | 0.40 | When a canvas texture is loaded and enabled, particles are steered into texture valleys. At 1.0 the fluid follows texture contours strongly, giving a paper-grain effect. |
